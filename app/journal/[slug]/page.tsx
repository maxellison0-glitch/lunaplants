...
export default async function JournalPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const post = getJournalPost((await params).slug);
  if (!post) notFound();
  const articleSchema = { "@context": "https://schema.org", "@type": "Article", headline: post.title, description: post.excerpt, datePublished: "2026-08-02", author: { "@type": "Organization", name: "Luna" }, publisher: { "@type": "Organization", name: "Luna" } };
  return (
    <main className="article-page">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }} />
      <header className="article-header"><div className="site-container"><Link className="article-back" href="/journal"><ArrowLeft size={15} /> Back to journal</Link><span className="eyebrow">{post.category} · {post.readTime}</span><h1>{post.title}</h1><p>{post.excerpt}</p><div><span>{post.date}</span><span>Written by Luna Studio</span></div></div></header>
      ...
      <section className="article-newsletter"><div><span className="eyebrow eyebrow-light">More useful things</span><h2>Notes for greener rooms.</h2></div><div><p>Get the next field note in your inbox, with 10% off your first Luna pairing.</p><Newsletter compact /></div></section>
    </main>
  );
}
