      ...
      <aside className={`cart-drawer ${drawerOpen ? "is-open" : ""}`} aria-hidden={!drawerOpen} aria-label="Shopping bag">
        ...
        {items.length === 0 ? (
          <div className="empty-bag">
            ...
            <p>Choose a plant, then make the pot your own.</p>
            <Link className="button button-primary" href="/shop" onClick={() => setDrawerOpen(false)}>Explore the collection</Link>
          </div>
        ) : (
          <>
              ...
              {items.map((item) => (
                <article className="drawer-item" key={item.id}>
                  <Link href={`/products/${item.slug}`} onClick={() => setDrawerOpen(false)}>
                    <ProductArt shape={item.shape} palette={item.palette} colour={item.colourHex} priorityLabel={`${item.name} in ${item.colour}`} />
                  </Link>
                  <div className="drawer-item-details">
                    <Link href={`/products/${item.slug}`} onClick={() => setDrawerOpen(false)}><h3>{item.name}</h3></Link>
                    <p>{item.size} · {item.colour}</p>
            ...
            <div className="drawer-footer">
              ...
              <p>UK delivery calculated at checkout. Free over £50.</p>
